...
Auth is wired up, and `lib/habits-context.tsx` / `lib/use-habit-history.ts` are backed by real Supabase queries against `habits` and `habit_completions` — the old `lib/dummy-habits.ts` / `lib/dummy-history.ts` generators are deleted, not just unused. A habit's `completed` state is derived from whether a `habit_completions` row exists for that habit today, not a stored boolean — same "existence means done" rule as the schema. No Supabase generated TypeScript types yet (`supabase gen types`, needs their CLI + project ref) — queries are written against the schema by hand.

- `supabase/schema.sql` — the source of truth for the database (`profiles`, `habits`, `habit_completions`, all RLS-scoped to `auth.uid()`). Run it in the Supabase SQL editor; it isn't applied automatically by anything in this repo.
- **This Next.js version renamed Middleware to Proxy** (verified against `node_modules/next/dist/docs` — "Starting with Next.js 16, Middleware is now called Proxy"). Session refresh + route protection live in root `proxy.ts` (exports `proxy()`, not `middleware()`) calling `updateSession()` in `lib/supabase/proxy.ts` — don't create a `middleware.ts`, it won't be picked up by this version.
- Two Supabase client factories, both named `createClient()` but not interchangeable: `lib/supabase/client.ts` (browser, Client Components) and `lib/supabase/server.ts` (Server Components/Actions, async, cookie-backed). Never share one client instance across requests/environments — always call the factory fresh.
- **Use `supabase.auth.getClaims()` for server-side auth checks, never `getSession()`** — `getClaims()` validates the JWT signature against the project's published keys; `getSession()` doesn't, and trusting it server-side is a known Supabase footgun (confirmed against their current docs, not assumed from older training knowledge).
- The protected-route list is an explicit allowlist in `lib/supabase/proxy.ts` (`protectedPrefixes`) — `/home`, `/history`, `/streaks`, `/stats`, `/profile`, `/create-habit`, `/edit-habit`. Add new protected routes there. Everything not listed (`/`, the PWA/manifest/icon routes, static assets) stays public by default — this is inverted from Supabase's own example code (which protects everything except an allowlist), chosen because our protected surface is the smaller, well-defined set.
- Env vars are `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` — Supabase renamed "anon key" to "publishable key" (confirmed current on their docs); don't reintroduce the old `_ANON_KEY` naming from memory.
- Sign-up stays intentionally disabled in the UI (`app/page.tsx`) — create test users via the Supabase dashboard (Authentication → Users) rather than building a sign-up flow, until that's explicitly scoped.
